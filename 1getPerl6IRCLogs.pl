...
my $urlstem = "http://irclog.perlgeek.de/perl6/";
my $forge_id = 65;
...
sub grabFiles($dbh1, $dbh2, $datasource_id, $urlstem, $date_to_start)
{
    ...
    while($date->ymd('') le $yesterday)
    {
		...
		# http://irclog.perlgeek.de/perl6/2015-06-03
		my $filestem = $yyyy . "-" . $mm . "-" . $dd ;
		...
		$insertQuery1->execute($newds, $forge_id, 'Perl6 IRC '. $yyyy.$mm.$dd, $saveLoc)
		...
		$insertQuery2->execute($newds, $forge_id, 'Perl6 IRC '. $yyyy.$mm.$dd, $saveLoc)
