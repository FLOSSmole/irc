...
my $urlstem = "http://django-irc-logs.com/";
my $forge_id = 42;
...
if ($datasource_id && $date_to_start)
{
	...
	my $dsn1 = "DBI:mysql:ossmole_merged:localhost";
	my $dbh1 = DBI->connect($dsn1, "user", "pass", {RaiseError=>1});
	
	my $dsn2 = "DBI:mysql:ossmole_merged:remotehost";
...
sub grabFiles($dbh1, $dbh2, $datasource_id, $urlstem, $date_to_start)
{
    ...
    while($date->ymd('') le $yesterday)
    {
		...
		my $filestem = $yyyy . "/" . $mm3{$mm} . "/" . $dd ;
		my $newURL = $p_urlstem . $filestem;
		print "getting URL $newURL\n";

		my $saveLoc = $p_datasource_id . "/" . $yyyy.$mm3{$mm}.$dd;
		...
		$insertQuery1->execute($newds, $forge_id, 'Django IRC '. $yyyy.$mm3{$mm}.$dd, $saveLoc)
