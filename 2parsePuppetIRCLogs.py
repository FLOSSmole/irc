...
db2 = MySQLdb.connect(host="grid6.cs.elon.edu",
                      user="megan",
                      passwd=pw,
                      db="ossmole_merged",
                      use_unicode=True,
...
db3 = MySQLdb.connect(host="flossdata.syr.edu",
                      user="megan",
                      passwd=pw,
                      db="puppet_irc",
                      use_unicode=True,
...
for row in rows:
    ...
    patternDateOfEntry = re.compile(ur'^(.*)\.txt$', re.UNICODE)
    ...
    for newLine in log:
        ...
        if (newLine[10:11] == "T"):
            pattern1 = re.compile(ur'^(.*?)T(.*?)\s\s(.*?)$', re.UNICODE)
        ...
        #[2013/06/22 20:59:24] @ lavaman joined channel #puppet-razor
            ...
            pattern2 = re.compile(ur'^\[(.*?) (.*?)\]\s(.*?)$', re.UNICODE)
        ...
        # 2014-02-10 10:39:18	 *	Dominic wonders if there's a language or
            ...
            if (restOfLine[0:1] == "*"):
                ...
                lineType = "system"
                pattern3s = re.compile(ur'\S+\s+(.*?)$', re.UNICODE)
                sendUser = None
                lineMessage = pattern3s.search(restOfLine).group(1)
            else:
                lineType = "message"
                pattern3m = re.compile(ur'(.*?)\s+(.*?)$', re.UNICODE)
